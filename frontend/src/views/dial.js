...
export function dialView(host, view) {
  ...
  const dial = () => host._runAction('dial', () => host._dialSmartValue(host._nodeInputDraft, host._pstnTrunkDraft));
  return html`<section class="dial-workspace">
    ...
    <label class="field"><span>Number, extension or node</span><div class="dial-input"><input id="node-input" autocomplete="off" .value=${host._nodeInputDraft} placeholder="Search a node or dial a number" @input=${event => {host._nodeInputDraft=event.target.value;host.requestUpdate();}} @keydown=${event => {if(event.key==='Enter') dial();}}><button class="primary" aria-label="Place call" ?disabled=${!view.connected || !host._nodeInputDraft.trim()} @click=${dial}>Call ↗</button></div></label>
    ...
    ${!targets.length ? html`<div class="empty compact">${host._targetsLoading ? 'Loading your contacts…' : 'Saved nodes and phones appear here. You can always dial above.'}</div>` : html`<div class="contacts">${repeat(targets, target => target.id, target => html`<button class="contact" ?disabled=${!view.connected} @click=${() => {
