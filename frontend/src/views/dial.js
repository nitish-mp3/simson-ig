...
export function dialView(host, view) {
  ...
  return html`<section class="dial-workspace">
    ...
    <label class="toggle node-video-toggle"><span><b>Video for node-to-node calls</b><small>Turn on once to grant camera access. SIP and gateway calls stay audio-only.</small></span><input type="checkbox" .checked=${host._videoEnabled} @change=${event => {host._videoEnabled=event.target.checked;host._saveMediaPreferences();host.requestUpdate();if(host._videoEnabled)host._refreshMediaDevices(true);}}></label>
    ${host._videoEnabled && !globalThis.isSecureContext ? html`<div class="notice error" role="status">Camera and microphone access require HTTPS. Open this dashboard using its secure address.</div>` : nothing}
    ${host._videoEnabled && host._turnAvailable === false ? html`<div class="notice error" role="status">No TURN relay is configured. Calls may fail on mobile or restrictive networks; ask the administrator to enable coturn.</div>` : nothing}
